    ...
    try:
        if event.type == VkBotEventType.MESSAGE_NEW and event.from_user:
            ...
            ## Опции и его дети (Подписки / отписки) (Сегодня / Завтра) (Экзамены)
                ...
                ## Дети "Опций"
            elif(dialstage == u.inScheduleMenu):
                    ## Подписки / отписки
                if(text == "подписаться"):
                    ...
                    ## Надькин
                    if(u.userid == 197096662):
                        u.send(f"ТЫ учись седня так, а завтра не завтра завтра:\n" + u.getSchedule(u.today()))
                    else:
            ...
            ## Установка группы
            elif(text == 'группа' and dialstage != u.inGroupSetup):
                u.dialstage = u.inGroupSetup
                u.send("Отправьте название вашей группы")

            elif(dialstage == u.inGroupSetup):
                try:
                    u.UserGroup = text
                    u.dialstage = u.inScheduleMenu
                    u.send("Выберите опцию")
